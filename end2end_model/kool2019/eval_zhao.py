...
from end2end_model.kool2019.utils.functions import parse_softmax_temperature
...
def eval_dataset(model,dataset, width, softmax_temp, opts):
    # param
    #   dataset: list[tensor[50,2],tensor[50,2],...,tensor[50.2]]
    #   直接传递dataset进来，不用再load了

    # Even with multiprocessing, we load the model here since it contains the name where to write results
    # 由于可能要执行多次，求解多个tsp问题，但模型只需要加载一次，因此，model加载放到外面，将加载好的model传入
    # model, _ = load_model(opts.model)
...
if __name__ == "__main__":
    ...
    # default opts values
    '''
    Namespace(datasets=['data/tsp/tsp20_test_seed1234.pkl'], f=False, o=None, val_size=10000, offset=0, eval_batch_size=1024, width=None, decode_strategy='greedy', softmax_temperature=1, model='pretrained/tsp_20', no_cuda=False, no_progress_bar=False, compress_mask=False, max_calc_batch_size=10000, results_dir='results', multiprocessing=False)
    '''
